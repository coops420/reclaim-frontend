...
import productImage from "../assets/sonicspillproof.jpg"; // Product image for Sonic Crushed Opal Spillproof
...
// Prices
const retailUSD = 475; // Crossed-out retail price for display
const saleUSD = 325;   // Final sale (Claim) price

// EmailJS credentials and templates
const serviceID = "service_c4kj8it";
// Use the dedicated seller template for Coopers Glass ("cooper_au2yp3c")
const sellerTemplateID = "cooper_au2yp3c";
const buyerTemplateID = "template_3mus039";
const publicKey = "BFAA9yJvj1yAllF9o";

const CoopersGlassProduct1 = () => {
  ...
  // 2) Generate the Solana Pay URL using the sale price (325 USD)
  useEffect(() => {
    if (claimPrice) {
      const totalClaimAmount = (saleUSD / claimPrice).toFixed(6);
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=Sonic%20Crushed%20Opal%20Spillproof&message=Purchase%20of%20Sonic%20Crushed%20Opal%20Spillproof`;
      setSolanaPayURL(url);
    }
  }, [claimPrice]);
  ...
  const handleSendConfirmation = () => {
    ...
    const templateParams = {
      ...
      email,
      shippingAddress,
      product: "Sonic Crushed Opal Spillproof",
      totalUSD,
      totalCLAIM,
      solanaPayURL,
    };

    // 3A) Send email to seller using the "cooper_au2yp3c" template and send to moviezzy@hotmail.com
    emailjs
      .send(
        serviceID,
        sellerTemplateID,
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="Sonic Crushed Opal Spillproof" className="product-image" />
        <h2 className="product-name">Sonic Crushed Opal Spillproof</h2>
...
export default CoopersGlassProduct1;
