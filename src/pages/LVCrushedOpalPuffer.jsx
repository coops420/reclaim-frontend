...
import { getClaimPrice } from "../components/TokenPrice"; // For live $CLAIM price
import "./ProductList.css";

// ** Assets (Vendors + Product images) **
import vendorImage from "../assets/coopersglass.jpg";  // Coopers Glass Vendor Image
import productImage from "../assets/lvpuffer.jpg";     // LV Crushed Opal Puffer

// ** ✅ QuickNode Mainnet RPC URL **
...
// ** Pricing Config for LV Crushed Opal Puffer **
const RETAIL_PRICE_USD = 425; // Retail price for display
const SALE_PRICE_USD   = 285; // Final sale (Claim) price
...
const LVPufferPurchase = () => {
  const { publicKey } = useWallet();

  // Store fetched $CLAIM price, shipping details, etc.
  ...
  // Calculate how much $CLAIM user needs
  const CLAIM_AMOUNT = claimPrice && claimPrice > 0
    ? (SALE_PRICE_USD / claimPrice).toFixed(6)
    : null;

  // If we have a valid price, user can buy
  const isReadyToBuy = CLAIM_AMOUNT !== null && !isNaN(CLAIM_AMOUNT) && CLAIM_AMOUNT > 0;

  // Buy function with signAndSendTransaction & shipping check
  const handleBuyNow = useCallback(async () => {
    ...
    try {
      ...
      // Ensure Buyer Has a $CLAIM Account
      try {
      ...
      // If recipient's token account is missing, create it
      ...
      // If we created an ATA, get a fresh blockhash for next transaction
      ...
      console.log(`Requesting Phantom confirmation to send ${CLAIM_AMOUNT} $CLAIM...`);
      ...
      // 3) Immediately send emails after successful payment
      const shippingAddress = `${address1}${address2 ? ", " + address2 : ""}, ${city}, ${stateProvince}, ${postalCode}, ${country}`;

      // Build EmailJS template
      const templateParams = {
        from_name:     fullName,
        email,
        shippingAddress,
        product:       "LV Crushed Opal Puffer",
        totalUSD:      SALE_PRICE_USD,
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="LV Crushed Opal Puffer" className="product-image" />
        <h2>LV Crushed Opal Puffer</h2>
        <p><del>Retail: ${RETAIL_PRICE_USD}</del></p>
        <p>
          {loading
            ? "Loading $CLAIM price..."
            : claimPrice
            ? `${SALE_PRICE_USD} USD ≈ ${CLAIM_AMOUNT} $CLAIM`
            : "N/A"}
        </p>
...
export default LVPufferPurchase;
