...
import productImage from "../assets/lvpuffer.jpg"; // LV Crushed Opal Puffer image
...
const retailUSD = 425; // Retail price for display
const saleUSD = 285;   // Final sale (Claim) price
...
const LVCrushedOpalPuffer = () => {
  ...
  // 2) Generate the Solana Pay URL using the sale price (285 USD)
  useEffect(() => {
    if (claimPrice) {
      const totalClaimAmount = (saleUSD / claimPrice).toFixed(6);
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=LV%20Crushed%20Opal%20Puffer&message=Purchase%20of%20LV%20Crushed%20Opal%20Puffer`;
      setSolanaPayURL(url);
    }
  }, [claimPrice]);
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        ...
        <h2 className="product-name">LV Crushed Opal Puffer</h2>
...
export default LVCrushedOpalPuffer;
