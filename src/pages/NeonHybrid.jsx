...
import productImage from "../assets/hybridneon.png"; // Product image for Electric Neon Hybrid Proxy Peak Spillproof
...
const retailUSD = 650; // Crossed-out retail price for display
const saleUSD = 450;   // Final sale (Claim) price
...
const NeonHybrid = () => {
  ...
  // We'll keep quantity state fixed at 1 since the selection is removed.
  const [quantity] = useState(1);

  // Global shipping details state
  ...
  // 2) Generate the Solana Pay URL using the sale price (450 USD)
  useEffect(() => {
    if (claimPrice) {
      const totalClaimAmount = (saleUSD / claimPrice).toFixed(6);
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=Electric%20Neon%20Hybrid%20Proxy%20Peak%20Spillproof&message=Purchase%20of%20Electric%20Neon%20Hybrid%20Proxy%20Peak%20Spillproof`;
      setSolanaPayURL(url);
    }
  }, [claimPrice, quantity]);
  ...
  const handleSendConfirmation = () => {
    ...
    const templateParams = {
      ...
      email,
      shippingAddress,
      product: "Electric Neon Hybrid Proxy Peak Spillproof",
      totalUSD,
      totalCLAIM,
      solanaPayURL,
    };
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="Electric Neon Hybrid Proxy Peak Spillproof" className="product-image" />
        <h2 className="product-name">Electric Neon Hybrid Proxy Peak Spillproof</h2>
