...
import productImage from "../assets/blackcropalpuffer.jpg"; // Product image for Sonic Crushed Opal Puffer
...
const retailUSD = 350; // Crossed-out retail price for display
...
const CoopersGlassProduct5 = () => {
  ...
  useEffect(() => {
    if (claimPrice) {
      ...
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=Sonic%20Crushed%20Opal%20Puffer&message=Purchase%20of%20Sonic%20Crushed%20Opal%20Puffer`;
      setSolanaPayURL(url);
    }
  }, [claimPrice, quantity]);
  ...
  const handleSendConfirmation = () => {
    ...
    const templateParams = {
      ...
      email,
      shippingAddress,
      product: "Sonic Crushed Opal Puffer",
      totalUSD,
      totalCLAIM,
      solanaPayURL,
    };

    // Send email to seller then to buyer
    emailjs
      .send(
        serviceID,
        sellerTemplateID,
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="Sonic Crushed Opal Puffer" className="product-image" />
        <h2 className="product-name">Sonic Crushed Opal Puffer</h2>
