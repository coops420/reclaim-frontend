...
import productImage from "../assets/neonpuffer.jpg";   // Product image for Electric Neon Puffer
...
const retailUSD = 325; // Crossed-out retail price for display
const saleUSD = 225;   // Final sale (Claim) price
...
const CoopersGlassProduct3 = () => {
  ...
  // 2) Generate the Solana Pay URL using the sale price (225 USD)
  useEffect(() => {
    if (claimPrice) {
      const totalClaimAmount = (saleUSD / claimPrice).toFixed(6);
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=Electric%20Neon%20Puffer&message=Purchase%20of%20Electric%20Neon%20Puffer`;
      setSolanaPayURL(url);
    }
  }, [claimPrice]);
  ...
  const handleSendConfirmation = () => {
    ...
    const templateParams = {
      ...
      email,
      shippingAddress,
      product: "Electric Neon Puffer",
      totalUSD,
      totalCLAIM,
      solanaPayURL,
    };

    // Send email to seller (to moviezzy@hotmail.com) then to buyer
    emailjs
      .send(
        serviceID,
        sellerTemplateID,
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="Electric Neon Puffer" className="product-image" />
        <h2 className="product-name">Electric Neon Puffer</h2>
...
export default CoopersGlassProduct3;
