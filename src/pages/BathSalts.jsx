...
import vendorImage from "../assets/gorillapackgenetics.webp"; // Changed to gorillapackgenetics
import bathsaltsImage from "../assets/bathsalts.jpg";         // Changed to bathsalts
...
// ** RECIPIENT WALLET (Updated to match second script) **
...
// ** EmailJS credentials and templates (updated seller template) **
const serviceID = "service_c4kj8it";
const sellerTemplateID = "template_5rj6drh";  // Changed from "cooper_au2yp3c"
const buyerTemplateID = "template_3mus039";
const publicKeyEmail = "BFAA9yJvj1yAllF9o";

// ------------------- NEW Price/Variant Fields from second script -------------------
// We'll maintain two "variants": "18ths" => $40 each, "zips" => $200 each.
const BathSalts = () => {
  ...
  // Shipping details (unchanged from first script)
  ...
  // Update USD price based on variant from second script
  useEffect(() => {
    if (variant === "18ths") {
      setUsdPrice(40);
    } else {
  ...
  // Compute how much $CLAIM is needed for total
  const totalUsdCost = usdPrice * quantity;
  const CLAIM_AMOUNT =
  ...
  // EXACT BUY LOGIC from first script, unchanged except for totalUsdCost
  const handleBuyNow = useCallback(async () => {
    ...
    try {
      ...
      // Convert $CLAIM to lamports based on totalUsdCost
      ...
      const templateParams = {
        ...
        email,
        shippingAddress,
        // Next line updated to "BathSalts" + variant:
  ...
  }, [CLAIM_AMOUNT, orderDetails, publicKey, quantity, totalUsdCost, variant]);
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        {/* Updated product image & name */}
        <img
          src={bathsaltsImage}
          alt={`BathSalts ${variant}`}
          className="product-image"
        />
        <h2>BathSalts {variant}</h2>
        {/* No "retail" line, replaced with dynamic variant price */}
        <p>
          {loading
            ? "Loading $CLAIM price..."
            : claimPrice
