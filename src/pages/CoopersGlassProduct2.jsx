...
import productImage from "../assets/lilactiptop.jpg"; // Product image for Lilac TipTop
...
// Prices
const retailUSD = 125; // Crossed-out retail price for display
const saleUSD = 95;    // Final sale (Claim) price
...
const CoopersGlassProduct2 = () => {
  ...
  // 2) Generate the Solana Pay URL using the sale price (95 USD)
  useEffect(() => {
    if (claimPrice) {
      const totalClaimAmount = (saleUSD / claimPrice).toFixed(6);
      const url = `solana:${recipientWallet}?amount=${totalClaimAmount}&spl-token=${CLAIM_TOKEN_ADDRESS}&label=Lilac%20TipTop&message=Purchase%20of%20Lilac%20TipTop`;
      setSolanaPayURL(url);
    }
  }, [claimPrice]);
  ...
  const handleSendConfirmation = () => {
    ...
    const totalUSD = saleUSD; // Final sale price is $95
    ...
    const templateParams = {
      ...
      email,
      shippingAddress,
      product: "Lilac TipTop",
      totalUSD,
      totalCLAIM,
      solanaPayURL,
    };

    // Send email to seller using "cooper_au2yp3c" template to moviezzy@hotmail.com
    emailjs
      .send(
        serviceID,
        sellerTemplateID,
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="product-details card" style={{ textAlign: "center" }}>
        <img src={productImage} alt="Lilac TipTop" className="product-image" />
        <h2 className="product-name">Lilac TipTop</h2>
...
export default CoopersGlassProduct2;
